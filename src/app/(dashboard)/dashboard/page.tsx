...
export default function DashboardPage() {
  ...
  const finalizados = data.filter((s) => s.situacao === "FINALIZADO").length;
  const cancelados = data.filter(
    (s) => s.situacao === "CANCELADO" || s.documento.includes("CANCELAMENTO")
  ).length;
  const pendentes = data.filter((s) => s.situacao === "PENDENTE").length;
  const atrasados = data.filter((s) => {
    if (s.situacao === "FINALIZADO" || s.situacao === "CANCELADO") return false;
    if (!s.prazo_envio) return false;
    return isBefore(new Date(s.prazo_envio), startOfDay(new Date()));
  }).length;

  // Gráficos
  const empreendimentosCount = data.reduce((acc, curr) => {
    acc[curr.empreendimento] = (acc[curr.empreendimento] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  const chartEmpreendimento = Object.entries(empreendimentosCount).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value);
